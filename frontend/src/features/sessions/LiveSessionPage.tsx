...
import { RunwayCharacterStage, type RunwayCharacterStageHandle } from "./RunwayCharacterStage";
...
export function LiveSessionPage({ sessionId }: LiveSessionPageProps) {
  ...
  const runwayRef = useRef<RunwayCharacterStageHandle>(null);
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      // 1. Disconnect Runway Character if connected
      if (runwayRef.current) {
        await runwayRef.current.disconnect();
      }

      // 2. Call backend runway/end (redundant but safe if disconnect didn't do it)
      try {
      ...
      // 3. Save manual transcript with endSession()
